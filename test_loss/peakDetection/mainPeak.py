...
def getPlotPlotIMTDiam (list,labelTag,pointsX,pointsY,savePathFig):
    ...
    for xpoint in pointsX:
        ax.axvline(xpoint, color='k', linestyle='--')
...
def getT(peaks):
    ...
    for i in xrange(len(peaks)-1):
        Tdelta = np.linalg.norm(peaks[i+1]-peaks[i])
        TList.append(Tdelta) 
    meanTarray  = np.mean(TList) 
    return meanTarray 
...
def main(root,SaveImage):
    ...
    x                     = find_peaks_cwt(getRandDiam, np.arange(1, 10))
    y                     = getRandDiam[x]
